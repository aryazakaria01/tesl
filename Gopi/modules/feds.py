...
import Gopi.modules.sql.feds_sql as sql
from Gopi import (EVENT_LOGS, LOGGER, OWNER_ID, DRAGONS, TIGERS, WOLVES,
                          dispatcher)
from Gopi.modules.disable import DisableAbleCommandHandler
from Gopi.modules.helper_funcs.alternate import send_message
from Gopi.modules.helper_funcs.chat_status import is_user_admin
from Gopi.modules.helper_funcs.extraction import (extract_unt_fedban,
                                                          extract_user,
                                                          extract_user_fban)
from Gopi.modules.helper_funcs.string_handling import markdown_parser
...
def new_fed(update: Update, context: CallbackContext):
    ...
    if fednam != '':
        ...
        if not x:
            update.effective_message.reply_text(
                "Can't federate! Please contact @cl_me_logesh if the problem persist."
            )
            return
...
def join_fed(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if not x:
            message.reply_text(
                "Failed to join federation! Please contact @cl_me_logesh should this problem persist!"
            )
            return
